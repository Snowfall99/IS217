...
sub iconview_react {
    ...
    if ( $value eq _( 'About' ) ) {
        ...
        return TRUE;
    } elsif ( $value eq _( 'Machine learning scan' ) ) {
        ml_select();
        return TRUE;
    } elsif ( $value eq _( 'Machine learning scan directory') ) {
        ml_select_directory();
        return TRUE;
    }
...
sub ml_select {
    ...
    if (-e $file) {
        ml_filter($file. FALSE, undef);
    }
}

sub ml_select_directory {
    my $directory = '';
    my $dialog    = Gtk3::FileChooserDialog->new(
        _( 'Select a directory' ), $window,
        'select-folder',
    ...
    if ( -e $directory ) {
        ml_filter( $directory, FALSE, undef );
    }
}

sub ml_filter {
    my ( $scanthis, $show, $from ) = @_;
    $from_cli = $from;
    ...
    $command = "/home/chenzx/Documents/virus/virus2.7/malware_test.sh";

    system($command, $scanthis);
    if ( $? == -1 )  
     {
